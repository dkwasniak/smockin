...
public class ServerSideEventServiceImpl implements ServerSideEventService {
    ...
    public void register(final String path, final long heartBeatMillis, final boolean proxyPushIdOnConnect, final Context ctx) throws IOException {
        ...
        liveLoggingHandler.broadcast(LiveLoggingUtils.buildLiveLogOutboundDTO(traceId, path, ctx.statusCode(), null, "SSE established (clientId: " + clientId + ")", false));
...
*/
    ...
    void applyHeaders(final Context ctx) {
       ...
       ctx.header(HttpHeaders.CONTENT_TYPE, SSE_EVENT_STREAM_HEADER);
       ctx.header(HttpHeaders.CACHE_CONTROL, "no-cache");
